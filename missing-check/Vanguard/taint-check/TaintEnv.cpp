...
TaintValueV visitCallExpr(CallExpr *E, Environment *env) {
    ...
    ASTFunction *call_function = common::call_graph->getFunction(E->getDirectCallee());
    ...
    if (TaintChecker::emit_arg) {
        TaintChecker::functions[env->F->getID()]->add_arg_values(call_function, values);
    }

    TaintValueV returnValue = TaintChecker::functions[call_function->getID()]->environment.return_value;
...
TaintValueV visitCXXConstructExpr(CXXConstructExpr *CXXE, Environment *env) {
    ...
    ASTFunction *call_function = common::call_graph->getFunction(CXXE->getConstructor());
    if (call_function != nullptr) {
...
TaintValueV visitStmt(Stmt *S, Environment *env) {
    ...
    if (DeclRefExpr *DRE = dyn_cast<DeclRefExpr>(S)) {
        ...
        if (V == nullptr) return TaintValueV();
    ...
    if (DeclStmt *DS = dyn_cast<DeclStmt>(S)) {
        for (Decl *D : DS->decls()) {
            ...
            if (VD == nullptr || !(VD->hasInit())) continue;
    ...
    if (BinaryOperator *E = dyn_cast<BinaryOperator>(S)) {
        ...
        if (E->isAssignmentOp()) {
            ...
            if (LValue.V != nullptr && LValue.V->isPointerOrReferenceType()) {
                (*env).values[LValue.V->getID()].update(RValue, true);
                return RValue;
            }
