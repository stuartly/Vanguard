...
/**
 * map:
 * VarDecl to ASTVariable
 * ASTFunction to FunctionDecl
 */
class ASTMap {

public:

    void buildMap(ASTUnit *AU, std::unordered_map<std::string, std::set<unsigned>> &filter_functions, bool build_variable = false);
...
private:
    ...
    void addVariable(ASTVariable *V, VarDecl *VD) {
        variableMap[VD] = V;
    }
...
public:
    ...
    ASTQueue();
    ASTUnit *push_back(std::string AST, std::unordered_set<std::string> &unique_functions, 
    ...
    void load(std::string AST, std::unordered_map<std::string, std::set<unsigned>> &filter_functions, 
            std::vector<ASTFunction *> &return_ast_functions, std::vector<FunctionDecl *> &return_functions);
    
    FunctionDecl *load(ASTFunction *F, std::unordered_map<std::string, std::set<unsigned>> &filter_functions);
