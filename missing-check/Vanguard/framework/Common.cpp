...
class FunctionDeclLoad: public ASTConsumer, public RecursiveASTVisitor<FunctionDeclLoad> {    
...
void initializeBlackWhite(int argc, const char *argv[]) {
    ...
    if (blackFile.is_open()) {
        taint::TaintChecker::blackList = common::loadFromSourceFile(path + "black.cpp");
    }
    blackFile.close();
    
    ifstream whiteFile(path + "white.cpp");
    if (whiteFile.is_open()) {
        taint::TaintChecker::whiteList = common::loadFromSourceFile(path + "white.cpp");
    }
    whiteFile.close();
}
