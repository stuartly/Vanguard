...
void DivChecker::check(ASTFunction *F, FunctionDecl *FD, CFG *function_cfg){
    ...
    if (test_function != "None" && test_function != "" && test_function != FD->getNameAsString()) {
        return;
    }
   ...
   // cerr <<"Check Expression start......." << std::endl;
	...
	//	cerr << "Binary........" << std::endl;
		...
		if(op == BinaryOperatorKind::BO_Assign){
            ...
            return 2;
		}else if (op == BinaryOperatorKind::BO_LAnd) {
            int lhs_result = checkExpression(ep, lhs, FuncName, checkFunc);
            int rhs_result = checkExpression(ep, rhs, FuncName, checkFunc);
            if (lhs_result == 0 || rhs_result == 0) return 0;
            if (lhs_result == 1 || rhs_result == 1) return 1;
            return 2;
        }else if (op == BinaryOperatorKind::BO_LOr) {
            int lhs_result = checkExpression(ep, lhs, FuncName, checkFunc);
            int rhs_result = checkExpression(ep, rhs, FuncName, checkFunc);
            if (lhs_result == 0 && rhs_result == 0) return 0;
            if (lhs_result == 1 && rhs_result == 1) return 1;
