...
{	
	...
	//if(DataNum!=0)
	//{
	//	std::string Dataxml = (*common::configure)["pathToReport"]["path"].asString() + "Label_Feature_Data.xml";
	//	Label_Feature_Data_Doc.save_file(Dataxml.c_str(),"\t", pugi::format_default|pugi::format_no_declaration);
	//}
...
bool TaintedMemOPChecker::is_dangerous_missing_check(ASTFunction* caller,CallExpr* MemOPcallExpr,FunctionDecl* callee)
{ 
	...
	string item=F1+F2+F3+F4+F5+F6+F7+F8+F9+F10+F11+F12+F13+F14+F15+"\n";

    //writingToFeatureData(item);

	ofstream myfile;
  	myfile.open ("PredicateData.txt",std::ios_base::app);
  	myfile << item.c_str();
  	myfile.close();
