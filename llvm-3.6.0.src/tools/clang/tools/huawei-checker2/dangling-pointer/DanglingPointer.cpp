...
{
    //cerr<<"var:"<<varName<<" added as "<<ifStatic<<" in "<<loc<<endl;
	if(loc == "")
		loc = "ERR:0:0";
...
{
    //cerr<<"fun:"<<name<<" added as arg:"<<arg << " in "<<loc<<endl;
	if(loc == "")
		loc = "ERR:0:0";
    refCount = 0;
    funName = name;
...
{
    //cerr<<"fun:"<<name<<" added as var:"<<varName << " in "<<loc<<endl;
    refCount = 0;
    funName = name;
    var = varName;
    argsNum = -1;
        ...
        if(VarDecl *E = dyn_cast<VarDecl>(D))
        {
            ...
            if(ifStaticVar(E->getQualifiedNameAsString(), &varIterator) == -1 && !E->isLocalVarDecl())
            {
				...
				if(loc != "")
				{
					...
					//cerr<<"VisitDecl-global:"<<tmp.varName<<endl;
...
int DanglingPtr::reportWarning(VarInfo &tmp, VarInfo* alias)
{
	...
	common::printLog(desc + "\n", common::CheckerName::danglingPointer, 5, *common::configure);
    return 0;
}
...
DanglingPtr::~DanglingPtr() {
	...
	if(warningCount != 0)
	{
		std::string xmlFile = (*common::configure)["pathToReport"]["path"].asString() + "MOLINT.POINTER.NULL.SET.xml";
		doc.save_file(xmlFile.c_str(), "\t", pugi::format_no_declaration);
	}
}

void DanglingPtr::check(ASTFunction *F, FunctionDecl *FD, CFG *function_cfg){
	...
	std::string funName = FD->getQualifiedNameAsString();
	if( function_cfg == NULL)
		return;
	common::printLog("Function: " + funName+ " begin\n", common::CheckerName::danglingPointer, 2, *common::configure);
	VisitFunDecl(FD, levelOfDete, function_cfg);
...
int DanglingPtr::readConfig(){
	//set level of detect and ifRegardParAsFreelike as fixed value.
	//levelOfDete = 1;
	//ifRegardParAsFreelike = true;
	
	if((*common::configure)["DanglingPointer"]["levelOfDete"].empty())
	{
		common::printLog( "Config: levelOfDete not found, using default value:2\n", common::CheckerName::danglingPointer, 3, *common::configure);
		levelOfDete = 2;
	}
	else
		levelOfDete = (*common::configure)["DanglingPointer"]["levelOfDete"].asInt();
	if((*common::configure)["DanglingPointer"]["ifRegardParAsFreelike"].empty())
	{
		common::printLog( "Config: ifRegardParAsFreelike not found, using default value:true\n", common::CheckerName::danglingPointer, 3, *common::configure);
		ifRegardParAsFreelike = true;
	}
	if((*common::configure)["DanglingPointer"]["ifRegardParAsFreelike"].asBool() == true)
		ifRegardParAsFreelike = true;
	else
		ifRegardParAsFreelike = false;
	
	for(auto mem:(*common::configure)["MemoryReleaseFunction"].getMemberNames())
	{
		memoryReleaseFuns[mem] = (*common::configure)["MemoryReleaseFunction"][mem].asInt();
	}
    return 0;
}
...
int DanglingPtr::varStateTransform(string lfh, string rfh, FunctionDecl *f, string loc, string fun, Expr* rhs)
{
        ...
        //cerr<<"binary oper:"<<lfh<<rfh<<" "<<ifStatic<<endl;
...
bool DanglingPtr::VisitFunDecl(FunctionDecl *f, int ttl, CFG *myCFG)
{
    if (f->hasBody()) {
        string funName = f->getQualifiedNameAsString();
		std::unordered_map<std::string, int>::iterator got = memoryReleaseFuns.find(funName);
		if(got != memoryReleaseFuns.end())
		{
			common::printLog(funName + " is memory release function, skipping.", common::CheckerName::danglingPointer, 2, *common::configure);
			return false;
		}
...
void DanglingPtr::printList()
{
    ...
    if(ifVar)
    {
		common::printLog("-------------VarInfo--------", common::CheckerName::danglingPointer, 1, *common::configure);
        list<VarInfo>::iterator iterator;
        for(iterator = vis.begin(); iterator != vis.end(); ++iterator)
        {
            if(iterator->freeLike)
				common::printLog(iterator->varName + "->" + iterator->freeLike->funName + "\n", common::CheckerName::danglingPointer, 1, *common::configure);
            else
				common::printLog(iterator->varName + "->" + iterator->freeLike->funName + "\n", common::CheckerName::danglingPointer, 1, *common::configure);
        }
    }
    if(ifFun)
    {
		common::printLog("-------------FunInfo--------", common::CheckerName::danglingPointer, 1, *common::configure);
        ...
        for(fiterator = fis.begin(); fiterator != fis.end(); ++fiterator)
        {
            if(fiterator->parent)
				...
				common::printLog(fiterator->funName + "->" + fiterator->parent->funName + " var:" + fiterator->var + "\n", common::CheckerName::danglingPointer, 1, *common::configure);
                //cerr<<fiterator->funName<<"->"<<fiterator->parent<<" var:"<<fiterator->var<<endl;

        }
    }
	common::printLog("--------------END-------", common::CheckerName::danglingPointer, 1, *common::configure);
...
void DanglingPtr::_DFSTopSort(ASTFunction *i, 
   ...
   const std::vector<ASTFunction *> children = common::call_graph->getChildren(i);
    if (children.size() != 0) {
...
std::vector<ASTFunction *> DanglingPtr::getNonTopoOrder() {
	...
	std::vector<ASTFunction *> topLevelFuncs = common::call_graph->getTopLevelFunctions();

    std::unordered_map<ASTFunction *, Color> colors; 

	//init colors
	for (auto func : common::resource->getFunctions()) {  
		colors[func] = WHITE;
	}
