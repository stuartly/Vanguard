...
#define ARRAYSIZE 5
void PreCheckDataSet::check(ASTFunction *F, FunctionDecl *FD, CFG *function_cfg)
{
	clock_t start = clock(); 
    readConfig();   
	if(F != NULL)
...
clock_t PreCheckDataSet::get_time() 
{    
    return run_time;
}
...
{		
	if(DataNum != 0)
	{
		std::string xmlFile = (*common::configure)["pathToReport"]["path"].asString() + "Label_Feature_Data.xml";
		Label_Feature_Data_Doc.save_file(xmlFile.c_str(),"\t", pugi::format_default|pugi::format_no_declaration);
	}
}
...
void PreCheckDataSet::visitStmt(ASTFunction *F, FunctionDecl *FD, CFG *function_cfg)
{	    
...
bool  PreCheckDataSet::has_Tainted_Arg(CallExpr* callExpr,string location_of_Arg_str)
{
...
bool PreCheckDataSet::is_Relevent_String(std::string if_condition_string,std::string Tainted_Data_Expr_string)
{
...
bool PreCheckDataSet::isSubTree(Node *bigTreeRoot,Node *smallTreeRoot) // if smallTree is the subtree of bigTree ?
{
...
bool PreCheckDataSet::is_Relevent_Expr(Expr* if_condition, Expr* Tainted_Data_Expr)
{
	Tree ifConditonTree;
...
bool PreCheckDataSet::is_Equal_String(std::string if_condition, std::string rightCheckStlye)
{
...
bool  PreCheckDataSet::is_right_ifCheck(string Tainted_MemOPName, unordered_map<Expr*, string> Tainted_Data_Expr_Set, unordered_map<Expr*, string> IfCheckInfoList) 
{
...
void PreCheckDataSet::collect_missing_check_data(ASTFunction* caller,CallExpr* MemOPcallExpr,FunctionDecl* callee, int label)
{ 
	...
	string item=F0+F1+F2+F3+F4+F5+F6+F7+F8+F9+F10+F11+F12+F13+F14+F15+"\n";
...
void PreCheckDataSet::writingToXML(std::string fileName, std::string funName, std::string descr, std::string locLine)
{
	WarningNum++;
