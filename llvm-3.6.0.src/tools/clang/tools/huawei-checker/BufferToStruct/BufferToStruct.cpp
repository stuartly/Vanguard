...
void BufferToStruct::check(ASTFunction *F, FunctionDecl *FD,
                           CFG *function_cfg) {
  clock_t start = clock();
  readConfig();
  if (F != NULL) {
...
clock_t BufferToStruct::get_time() { return run_time; }

BufferToStruct::BufferToStruct() { WarningNum = 0; }

BufferToStruct::~BufferToStruct() {
  if (WarningNum != 0) {
    std::string xmlFile =
        (*common::configure)["pathToReport"]["path"].asString() +
        "MOLINT.BUFFET.TO.STRUCT.xml";
...
void BufferToStruct::visitStmt(ASTFunction *F, FunctionDecl *FD,
                               CFG *function_cfg) {
...
void BufferToStruct::writingToXML(std::string fileName, std::string funName,
