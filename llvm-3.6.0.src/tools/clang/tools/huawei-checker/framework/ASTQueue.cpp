...
void ASTMap::buildMap(
    ASTUnit *AU,
    std::unordered_map<std::string, std::set<unsigned>> &filter_functions,
    bool build_variable) {

  const std::vector<ASTFunction *> &ast_functions =
      common::resource->getFunctions(AU->getASTFileName());
  ...
  if (filter_functions.count(AU->getASTFileName()) != 0) {
    assert(functions.size() ==
           ast_functions.size() +
               filter_functions[AU->getASTFileName()].size());
  } else
    assert(functions.size() == ast_functions.size());
  ...
  while (i < functions.size()) {
    ...
    if (build_variable) {
      const std::vector<ASTVariable *> &ast_variables =
          ast_functions[j]->getVariables();
      const std::vector<VarDecl *> variables =
          common::getVariables(functions[i]);
      unsigned m = 0, n = 0;
