...
ASTManager::ASTManager(std::vector<std::string> &ASTFiles) {
  ...
  for (std::string AST : ASTFiles) {
    ...
    priorityQueue.push_back(AST, unique_functions, filter_functions, functions,
                            ast_functions);
    ...
    for (unsigned i = 0; i < functions.size(); i++) {
      ...
      CFG *function_cfg =
          priorityQueue.maps[AST]->getCFG(ast_functions[i], functions[i]);
...
void ASTManager::clear() { current_size = 0; }
...
void ASTManager::push(std::string AST) {
  ...
  maps[AST]->buildMap(au_point, filter_functions);
}

void ASTManager::load(std::string AST,
                      std::vector<ASTFunction *> &ast_functions,
                      std::vector<FunctionDecl *> &functions) {
