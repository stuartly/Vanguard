...
BlockEnvironment::BlockEnvironment(ASTFunction *F, unsigned blockID,
                                   bool is_entry)
    : environment(F, is_entry) {
...
FunctionEnvironment::FunctionEnvironment(ASTFunction *F, FunctionDecl *FD,
                                         CFG *function_cfg)
    : environment(F) {
...
void FunctionEnvironment::add_arg_values(ASTFunction *F,
...
void FunctionEnvironment::emit_param_values() {
  ...
  for (auto &content : *arg_values) {
    ...
    TaintChecker::functions[F->getID()]->param_values |= value;
  }
}

Environment
FunctionEnvironment::getPredEnvironment(CFGBlock *block,
...
void FunctionEnvironment::visitCFGBlocks(CFG *function_cfg) {
  ...
  is_visited = true;
  environment = outs[function_cfg->getExit().getBlockID()];
}
}
