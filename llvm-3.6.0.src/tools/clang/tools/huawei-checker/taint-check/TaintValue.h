...
public:
  ...
  TaintValue(unsigned v) { value = v; }

  TaintValue(TaintKind kind) {
    if (kind == TaintKind::Untainted)
      value = 0;
    else if (kind == TaintKind::Tainted)
      value = UINT_MAX;
    else
      value = 1;
  }
  ...
  bool isTainted() const { return value > 0; }
  ...
  void update(unsigned value) { this->value = value; }
  void update(const TaintValue &v) { value = v.value; }
...
public:
  ...
  bool is_tainted() const { return value->isTainted(); }

  TaintKind getTaintKind() const { return value->getTaintKind(); }

  unsigned getValue() const { return value->getValue(); }
  ...
  friend TaintValue combineValues(TaintValueV value,
                                  std::vector<TaintValueV> values);

private:
  std::shared_ptr<TaintValue> value;

  void setValue(unsigned v) { value->update(v); }
};
