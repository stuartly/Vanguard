...
struct ExprPosition {
  ...
  Stmt *stmt;
  Expr *expr;
  ExprPosition(FunctionDecl *f = nullptr, CFGBlock *b = nullptr,
               Stmt *s = nullptr, Expr *e = nullptr) {
    function = f;
    block = b;
    stmt = s;
    expr = e;
  ...
  ExprPosition(const FunctionDecl *f, const CFGBlock *b, const Stmt *s,
               const Expr *e) {
  ...
  void dump() {
    ...
    expr->printPretty(exprOS, nullptr,
                      PrintingPolicy(function->getASTContext().getLangOpts()));
  ...
  friend OStream &operator<<(OStream &OS, const ExprPosition &ep) {
    ...
    if (ep.block)
      OS << "stmt: " << common::getStringStmt(
                            ep.stmt, ep.function->getASTContext().getLangOpts())
         << "\n";

    if (ep.block)
      OS << "expr: " << common::getStringStmt(
                            ep.expr, ep.function->getASTContext().getLangOpts())
         << "\n";
